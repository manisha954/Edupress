...
import CompanyInfoForm from "./AddHeroSectionDetails";
import { getHeroSectionDetails } from "./HeroSectionAction";
...
export default function WebsiteDetailTable() {
  ...
  const { deleteHeroSection } = useHeroSectionMutations();

  const { data: heroSectionData, error } = useQuery({
    queryKey: [" heroSection"],
    queryFn: async () => getHeroSectionDetails(),
  });

  console.log(" heroSection", heroSectionData);

  useEffect(() => {
    if (heroSectionData?.success) {
      setNavItems(heroSectionData?.data);
    } else {
      setNavItems([]);
    }
  }, [heroSectionData]);

  const handleEdit = (heroSection: any) => {
    setEditItem(heroSection);
    setOpenAddNav(true); // Open the form for editing
  };

  const handleDelete = async (heroSectionId: string) => {
    const deleteNav = await deleteHeroSection(heroSectionId);
    if (deleteNav?.success) {
      setSuccessMessage(deleteNav?.success);
      setNavItems((prev: any) =>
        prev.filter((item: any) => item.heroSectionId !== heroSectionId)
      );
    } else {
      setErrorMessage("Error deleting  heroSection");
    }
  };
  ...
  const filteredHeroSectionDetails = navItems?.filter((heroSection: any) =>
    heroSection?.companyName?.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  const currentItems = filteredHeroSectionDetails.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
    ...
    <div className="bg-white  px-2">
        ...
        <div className="flex justify-end ">
          <button
            ...
            onClick={() => {
              ...
              setOpenAddNav(true); // Open the form for adding a new  heroSection
            }}
          >
            New HeroSection
          </button>
        ...
        <div className="w-full ">
          ...
          <div className="block md:hidden rounded-lg">
            <div className="flex-1 overflow-auto h-[55vh] space-y-4 p-4">
              {currentItems.map((heroSection: any) => (
                <div
                  key={heroSection?.heroSectionId}
                ...
                >
                  ...
                  <div className=" gap-2 grid grid-cols-2">
                    ...
                    <ConfirmButton
                      ...
                      buttonType="delete"
                      userId={heroSection?.heroSectionId}
                    ...
                    onClick={() => handleDelete(heroSection?.heroSectionId)}
                  >
                    Delete
                  </button> */}
                  </div>
                  ...
                  <div className="divide-y relative h-[66vh] md:h-[66vh] divide-gray-200 rounded-lg border dark:divide-gray-700 dark:border-gray-700">
                    <table className=" min-w-full divide-y divide-gray-200 ">
                      ...
                      <tbody className="divide-y divide-gray-500 dark:divide-gray-700">
                        {filteredHeroSectionDetails.map((heroSection: any) => (
                          <tr
      ...
      <div className="mt-2">
        <PaginationTable
          ...
          data={filteredHeroSectionDetails}
